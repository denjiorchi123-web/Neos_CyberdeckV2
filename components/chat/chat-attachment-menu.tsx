  ...
  const options = [
    {
      ...
      color: "bg-indigo-500",
      onClick: () => onOpen("messageFile", { apiUrl, query, fileType: "document" })
    },
    {
      label: "Camera",
      icon: <Camera className="h-6 w-6 text-white" />,
      color: "bg-pink-500",
      onClick: () => onOpen("messageFile", { apiUrl, query, fileType: "camera" })
    },
    {
      label: "Gallery",
      icon: <ImageIcon className="h-6 w-6 text-white" />,
      color: "bg-purple-500",
      onClick: () => onOpen("messageFile", { apiUrl, query, fileType: "image" })
    },
    {
      label: "Audio",
      icon: <Headphones className="h-6 w-6 text-white" />,
      color: "bg-orange-500",
      onClick: () => onOpen("messageFile", { apiUrl, query, fileType: "audio" })
    },
    {
      label: "Location",
