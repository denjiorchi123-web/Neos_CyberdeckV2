...
export function CommunityMembersModal() {
  ...
  const isModalOpen = isOpen && type === "communityMembers";
  const { community } = data;

  const onKick = async (memberId: string) => {
    try {
      setLoadingId(memberId);
      const url = `/api/communities/${community.id}/members/${memberId}`;
      const response = await axios.delete(url);
      router.refresh();
      onOpen("communityMembers", { community: response.data });
    } catch (error) {
      console.error(error);
    } finally {
      setLoadingId("");
          ...
          {community?.members?.map((member: any) => (
            <div key={member.id} className="flex items-center gap-x-2 mb-6 px-6">
              ...
              <div className="flex flex-col gap-y-1">
                ...
                <p className="text-xs text-zinc-500">{member.profile.email || member.profile.name}</p>
              </div>
